...
static unsigned int ext4_count_free(char *map, unsigned int numchars, unsigned int numremaind)
{
	...
	for (i = 0; i < numremaind; i++) {
		if (((map[numchars]>>i) & 0x01) == 0)
			sum++;
	}
	return sum;
}

static errcode_t read_block_bitmaps(e2fsck_t ctx, char * buf)
{
	int i;
	...
	for (i = 0; i < (int)fs->group_desc_count; i++) {
		...
		retval = io_channel_read_blk64(fs->io, blk,
						   1, buf+i*fs->blocksize);
		if (retval) {
			log_out(ctx, "read bitmap %dth group, bitmap loc : %llu\n", i, blk);
			return 1;
		}
	}
	return 0;
...
static int check_free_count(e2fsck_t ctx)
{
	ext2_filsys fs = ctx->fs;
	struct ext2_super_block *es = fs->super;
	int i;
	int block_nbytes = EXT2_CLUSTERS_PER_GROUP(es) / 8;
	blk64_t blk_itr = EXT2FS_B2C(fs, es->s_first_data_block);
	char *block_buf = NULL;
	errcode_t retval;
	int csum_flag = 0;
	...
	if (EXT2_HAS_RO_COMPAT_FEATURE(es,
	...
	if (fs->block_map) {
		...
		/* read only block bitmap buffer directly, do not create any bitmap struct */
		retval = ext2fs_get_mem(fs->blocksize*(fs->group_desc_count), &buf);
		if (retval) {
			log_out(ctx, "bitmap alloc fail\n");
			return 1;
		}
		retval = read_block_bitmaps(ctx, buf);
		if (retval) {
			log_out(ctx, "read bitmap fail\n");
			return 1;
		}
	}
...
static int release_orphan_inodes(e2fsck_t ctx)
{
	 ...
	 * with EXT2_ERROR_FS superblock flags */
	fs_state = fs->super->s_state;

	fs->super->s_state = EXT2_ERROR_FS;
	ext2fs_mark_super_dirty(fs);
	log_out(ctx, _("superblock fsync before orphan cleanup!\n"));
	...
	ext2fs_mark_super_dirty(fs);
	log_out(ctx, _("superblock fsync after orphan cleanup!\n"));
	retval = write_primary_superblock(fs, fs->super);
	if (retval) {
		log_out(ctx, _("superblock write failed! retval = %ld\n"),retval);
		goto err;
...
#define API_LEVEL_SOS 31
	if (sb->s_snapshot_r_blocks_count != API_LEVEL_SOS) {
		log_out(ctx, _("s_snapshot_r_blocks_count is %llu."
			" It might be upgraded from R OS. Reinitialize it to 31\n"),
			(unsigned long long)sb->s_snapshot_r_blocks_count);
		sb->s_snapshot_r_blocks_count = API_LEVEL_SOS;
	...
	if (!ext2fs_has_feature_64bit(fs->super) &&
		...
		if (fix_problem(ctx, PR_0_COUNT_HIGH_WITHOUT_64BIT, &pctx)) {
			fs->super->s_blocks_count_hi   = 0;
			fs->super->s_r_blocks_count_hi = 0;
			fs->super->s_free_blocks_hi    = 0;
			ext2fs_mark_super_dirty(fs);
		}
	}
	...
	/* 
	 * WA: 200604-03255
	 * ignore checking last write time in super block
	 * In case of Qcom models, after reboot, e2fsck checks
	 * the write time in super block and it always represent the future.
	 * To avoid this problem, we remove this checking code.
	 */
	if (!broken_system_clock &&
	    !(ctx->flags & E2F_FLAG_TIME_INSANE) &&
