...
static void write_debug_code(ext2_filsys fs, int i)
{
	int retval = 0;
	char *magic = calloc(SUPERBLOCK_SIZE, 1);

	if (!magic) {
		printf("Failed to allocate magic\n");
		return;
	}

	snprintf(magic, SUPERBLOCK_SIZE,
			"%s mke2fs_discard_device", i?"After ":"Before");

	io_channel_set_blksize(fs->io, SUPERBLOCK_OFFSET);
	retval = io_channel_write_blk64(fs->io, 1, -SUPERBLOCK_SIZE, magic);
	if (!retval)
		retval = io_channel_flush(fs->io);
	io_channel_set_blksize(fs->io, fs->blocksize);

	if (retval)
		printf("Failed to write debug message at superblock\n");
...
static int mke2fs_discard_device(ext2_filsys fs)
{
	...
	blk64_t cur;
	int retval = 0;
	int offset = SUPERBLOCK_OFFSET/fs->blocksize + 1;
	clock_t start = clock();

	/* To debug in case of sudden power off while formatting */
	zap_sector(fs, 0, 2);
	zap_sector(fs, 4, 4);
	write_debug_code(fs, 0);
	 ...
	 * afterwards.
	 */
	retval = io_channel_discard(fs->io, offset, fs->blocksize - offset);
	if (retval)
		return retval;
	cur = fs->blocksize;
	...
	if (retval) {
		...
		ext2fs_numeric_progress_close(fs, &progress,
				      _("done                            \n"));
		write_debug_code(fs, 1);
	}

	printf("Discard takes %5.5fs.\n", (double)(clock() - start)/CLOCKS_PER_SEC);
...
int main (int argc, char *argv[])
{
	...
	/* Initialze s_snapshot_r_blocks_count to API_LEVEL_SOS */
#define API_LEVEL_SOS 31
	fs->super->s_snapshot_r_blocks_count = API_LEVEL_SOS;
