...
static void lzo_compress_init(struct compress_ctx *cc)
{
	...
	cc->_private = malloc(alloc);
	ASSERT(cc->_private);
	cc->rbuf = (char *) cc->_private + LZO_WORK_SIZE;
...
static void lz4_compress_init(struct compress_ctx *cc)
{
	...
	cc->_private = malloc(alloc);
	ASSERT(cc->_private);
	cc->rbuf = (char *) cc->_private + LZ4_WORK_SIZE;
	cc->cbuf = (struct compress_data *)((char *) cc->rbuf + size);
}

static int lz4_compress(struct compress_ctx *cc)
{
	cc->clen = LZ4_compress_fast_extState(cc->_private, cc->rbuf,
...
static void pc_filter_add(const char *path)
{
	FILE *fp;
	char s[PATH_MAX];
	re_t *node;
	...
	fp = fopen(path, "r");
	if (!fp) {
		fprintf(stderr, "%s: can not open %s: %d.. give up partial "
				"compression\n", __func__, path, errno);
		ASSERT(0);
	}

	while (fgets(s, PATH_MAX, fp)) {
		int ret;

		s[strlen(s) - 1] = '\0';

		node = (re_t *) malloc(sizeof(re_t));
		ASSERT(node);

		ret = regcomp(&node->re, s, REG_EXTENDED);
		if (ret) {
			fprintf(stderr, "%s: can not compile regex for %s\n",
					__func__, s);
			ASSERT(0);
		}
		node->next = re_list;
...
static void pc_filter_destroy(void)
{
	re_t *p;
...
static bool pc_do_filter(const char *path)
{
	re_t *p = re_list;

	while (p) {
		if (!regexec(&p->re, path, 0, NULL, 0)) {
			printf("%s: don't compress %s\n", __func__, path);
			return false;
		}
		p = p->next;
	}
...
filter_ops pc_filter = {
	.add = pc_filter_add,
	.destroy = pc_filter_destroy,
	.filter = pc_do_filter,
};
