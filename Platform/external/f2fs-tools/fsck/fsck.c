			...
			struct node_info *ni)
{
	...
	if (!IS_VALID_BLK_ADDR(sbi, ni->blk_addr)) {
		ASSERT_MSG("blkaddress is not valid. ni->ino [0x%x], blk_addr [0x%x]", ni->ino, ni->blk_addr);
		return -EINVAL;
	}
	...
	if ((c.feature & cpu_to_le32(F2FS_FEATURE_COMPRESSION)) && ntype == TYPE_INODE &&
	    node_blk->i.i_flags & cpu_to_le32(F2FS_COMPR_FL) &&
	    F2FS_FITS_IN_INODE(&(node_blk->i), le16_to_cpu(node_blk->i.i_extra_isize), i_log_cluster_size)) {
		if ((node_blk->i.i_compress_algrithm >= MAX_COMPRESS_ALGS) ||
		    (le64_to_cpu(node_blk->i.i_compr_blocks) > le64_to_cpu(node_blk->i.i_blocks)) ||
		    (node_blk->i.i_log_cluster_size < MIN_COMPRESS_LOG_SIZE) ||
		    (node_blk->i.i_log_cluster_size > MAX_COMPRESS_LOG_SIZE)) {
			ASSERT_MSG("ino[0x%x] has invalid compression info.", ni->ino);
			return -EINVAL;
		}
	}
		...
		u32 *blk_cnt, struct child_info *child)
{
	...
	if (sanity_check_nid(sbi, nid, node_blk, ftype, ntype, &ni)) {
		if (nid == sbi->root_ino_num) {
			MSG(0, "invalid root inode(ino:%d)\n", sbi->root_ino_num);
			exit(EXIT_FAILURE);
		}
		goto err;
	}
...
static int is_inline_data(struct f2fs_sb_info *sbi, nid_t nid)
{
	struct node_info ni;
	struct f2fs_node *node_blk;
	int ret = 0;
		...
		int max, int idx, int last_blk, int enc_name)
{
	...
	if (c.show_file_map) {
		struct f2fs_dentry *d = fsck->root_dentry;

		if (dentry[idx].file_type != F2FS_FT_REG_FILE)
			return;

		/* need to check if it is inline or not */
		if (is_inline_data(sbi, le32_to_cpu(dentry[idx].ino)))
			return;
...
static int quota_inode_init_new(struct node_info ni)
{
	int ret;
	struct f2fs_node *node_blk = NULL;
	...
	node_blk->footer.nid = ni.ino;
	node_blk->footer.ino = ni.ino;
	node_blk->footer.cp_ver = cpu_to_le64(0xbab0);
	node_blk->footer.next_blkaddr = cpu_to_le32(0xcafe);

	node_blk->i.i_mode = cpu_to_le16(0x8180);
	node_blk->i.i_links = cpu_to_le32(1);

	node_blk->i.i_blocks = cpu_to_le64(1);
	node_blk->i.i_atime = cpu_to_le32(time(NULL));
	node_blk->i.i_ctime = cpu_to_le32(time(NULL));
	node_blk->i.i_mtime = cpu_to_le32(time(NULL));
	node_blk->i.i_flags = FS_IMMUTABLE_FL;
	node_blk->i.i_dir_level = DEF_DIR_LEVEL;
	...
	if (c.feature & cpu_to_le32(F2FS_FEATURE_PRJQUOTA))
		node_blk->i.i_projid = cpu_to_le32(F2FS_DEF_PROJID);
...
static int get_valid_quota_ino(struct f2fs_sb_info *sbi, u32 ino,
			       struct node_info *ni, enum quota_type qtype)
{
	struct f2fs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct f2fs_nm_info *nm_i = NM_I(sbi);
	nid_t i;
	if (IS_VALID_NID(sbi, ino) && ino > sbi->root_ino_num && ino == ni->ino)
		return 0;

	/* Need to allocate nid */
	for (i = sbi->root_ino_num + 1; i < nm_i->max_nid; i++)
		if(f2fs_test_bit(i, nm_i->nid_bitmap) == 0)
			break;

	if (i >= nm_i->max_nid) {
		ASSERT_MSG("Failed to allocate quota inode, qtype [%d] ino [0x%x] is invalid",
							qtype, ino);
		/* Disable quota inode */
		sb->qf_ino[qtype] = 0;
	...
	/* Update node_info to newly allocated nid */
...
static int get_valid_quota_blkaddr(struct  f2fs_sb_info *sbi,
			struct node_info *ni, enum quota_type qtype)
{
	struct f2fs_super_block *sb = F2FS_RAW_SUPER(sbi);
	...
	if (ni->blk_addr >= SM_I(sbi)->main_blkaddr &&
		ni->blk_addr < le64_to_cpu(F2FS_RAW_SUPER(sbi)->block_count))
		return 0;

	set_summary(&sum, ino, 0, ni->version);
	err = reserve_new_block(sbi, &ni->blk_addr, &sum, CURSEG_HOT_NODE, 1);
	if (err) {
		ASSERT_MSG("Failed to allocate quota inode block, qtype [%d] ino [0x%x]",
							qtype, ino);
		/* Disable quota inode */
		sb->qf_ino[qtype] = 0;
		update_superblock(sb, SB_MASK_ALL);
	...
	update_nat_blkaddr(sbi, ino, ino, ni->blk_addr);
	get_node_info(sbi, ino, ni);
...
int fsck_chk_quota_node(struct f2fs_sb_info *sbi)
{
	...
	for (qtype = 0; qtype < F2FS_MAX_QUOTAS; qtype++) {
		...
		if (ret) {
			...
			/* Case 1. sb->qf_ino[qtype] has been corrupted */
			err = get_valid_quota_ino(sbi, ino, &ni, qtype);
			if (err)
				return -ENOSPC;

			/* Case 2. quota inode is valid but blk_addr has been corrupted */
			err = get_valid_quota_blkaddr(sbi, &ni, qtype);
			if (err)
				return -ENOSPC;

			c.fix_on = 1;
			quota_inode_init_new(ni);

			err = fsck_chk_node_blk(sbi, NULL, ni.ino,
					F2FS_FT_REG_FILE, TYPE_INODE, &blk_cnt, NULL);
			if (err) {
				ASSERT_MSG("Failed to fix quota inode, qtype [%d] ino [0x%x]",
									qtype, ni.ino);
				/* Disable quota inode */
				sb->qf_ino[qtype] = 0;
				update_superblock(sb, SB_MASK_ALL);
...
void fsck_read_extra_flag_blk(struct f2fs_sb_info *sbi)
{
	struct f2fs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	block_t extra_blk_no = le32_to_cpu(sb->cp_blkaddr) - 1;
	int res;

	if (extra_blk_no < 2) {
		DBG(1, "No free blks for extra_flag_blk\n");
		return;
	}
	fsck->extra_flag_blkno = extra_blk_no;

	res = dev_read_block(&fsck->extra_blk, extra_blk_no);
	if (res < 0) {
		ERR_MSG("Failed to read extra_flag_blk\n");
		memset(&fsck->extra_blk, 0, sizeof(struct f2fs_extra_flag_blk));
	}
}

int fsck_chk_need_fsck(struct f2fs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	struct f2fs_extra_flag_blk *extra_blk = &fsck->extra_blk;
	int ret = 0;

	if (le32_to_cpu(extra_blk->need_fsck) == F2FS_SEC_FSCK_MAGIC) {
		stlog("Info: found force FSCK flag!\n");
		ret = 1;
	}

	if ((unsigned)(le32_to_cpu(extra_blk->spo_counter) >= FSCK_SPO_CNT_THR))
		ret = 1;

	if (ret)
		stlog("Info: force checking by extra flag block\n");
	return ret;
}

/*
 * fsck_clear_extra_flag_blk initializes values
 * that need to be cleaned after full check.
 */
static void fsck_clear_extra_flag_blk(struct f2fs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	struct f2fs_extra_flag_blk *extra_blk = &fsck->extra_blk;

	extra_blk->need_fsck = 0;
	extra_blk->spo_counter = 0;
}

void fsck_set_extra_fs_info(struct f2fs_sb_info *sbi)
{
	struct f2fs_checkpoint *cp = F2FS_CKPT(sbi);
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	struct f2fs_extra_flag_blk *extra_blk = &fsck->extra_blk;

	extra_blk->valid_node_count = get_cp(valid_node_count);
	extra_blk->valid_inode_count = get_cp(valid_inode_count);
}

void fsck_update_extra_blk(struct f2fs_fsck *fsck)
{
	struct f2fs_extra_flag_blk *extra_blk = &fsck->extra_blk;
	block_t extra_blk_no = fsck->extra_flag_blkno;
	int res;
	...
	extra_blk->fsck_read_bytes = cpu_to_le64(c.rbytes);
	extra_blk->fsck_written_bytes = cpu_to_le64(c.wbytes);
	extra_blk->fsck_elapsed_time = cpu_to_le64(c.elapsed_time);
	extra_blk->fsck_exit_code = cpu_to_le32(c.exit_code);

	res = dev_write_block(extra_blk, extra_blk_no);
	if (res < 0)
		ERR_MSG("Failed to write extra_flag_blk\n");

	res = f2fs_fsync_device();
	if (res < 0)
		ERR_MSG("Failed to fsync extra_flag_blk\n");
}

static u64 get_max_image_size(struct f2fs_sb_info *sbi)
{
	struct f2fs_checkpoint *cp = F2FS_CKPT(sbi);
	u32 valid_main_segs;
	u64 max_image_segs;
	...
	valid_main_segs = (sbi->total_sections * sbi->segs_per_sec) -
						get_cp(free_segment_count);
	sit_segs = SEG_ALIGN(SIZE_ALIGN(valid_main_segs, SIT_ENTRY_PER_BLOCK));
	sit_segs *= 2;
	nat_segs = SEG_ALIGN(SIZE_ALIGN(get_cp(valid_node_count), NAT_ENTRY_PER_BLOCK));
	nat_segs *= 2;
	max_image_segs = sb_seg + cp_segs + sit_segs + nat_segs + valid_main_segs;

	if (max_image_segs < (F2FS_MIN_SEGMENTS + 1))
		return (F2FS_MIN_SEGMENTS + 1) * sbi->blocks_per_seg * sbi->blocksize;
	else
		return max_image_segs * sbi->blocks_per_seg * sbi->blocksize;
}
...
int fsck_verify(struct f2fs_sb_info *sbi)
{
	...
	printf("[FSCK] Max image size: %"PRIu64" MB, Free space: %u MB\n",
		get_max_image_size(sbi) >> 20,
		(sbi->user_block_count - sbi->total_valid_block_count) >>
		(20 - F2FS_BLKSIZE_BITS));
	printf("[FSCK] Max node count: %"PRIu32" / %"PRIu32"\n",
			fsck->chk.valid_node_cnt, sbi->total_node_count);
	...
	if (force || (c.fix_on && f2fs_dev_is_writable())) {
		...
		if (force || c.bug_on || c.bug_nat_bits) {
			...
			fix_checksum(sbi);
			fix_checkpoints(sbi);
			stlog("Info: FILE SYSTEM WAS MODIFIED (fixed)\n");
		} else if (is_set_ckpt_flags(cp, CP_FSCK_FLAG) ||
			is_set_ckpt_flags(cp, CP_QUOTA_NEED_FSCK_FLAG)) {
			write_checkpoints(sbi);
			stlog("Info: FILE SYSTEM WAS MODIFIED (check only)\n");
		}

		fsck_clear_extra_flag_blk(sbi);
		/* to return FSCK_ERROR_CORRECTED */
		ret = 0;
	}
	stlog("Info: %s %u files, %u/%u nodes, %llu/%llu blocks, %u SPO cnt\n",
			c.devices[0].path, fsck->chk.valid_inode_cnt,
			fsck->chk.valid_node_cnt, sbi->total_node_count,
			fsck->chk.valid_blk_cnt, sbi->user_block_count,
			le32_to_cpu(fsck->extra_blk.spo_counter));
	fsck_set_extra_fs_info(sbi);
	return ret;
}
