...
void resize_usage()
{
	...
	MSG(0, "  -F [0|1] fail-safe resize (Does not move main blocks) [default:1]\n");
...
void sload_usage()
{
	...
	MSG(0, "    -n regular expression file for no compression(allowlist)\n");
	MSG(0, "    -m <num> min compressed blocks per cluster\n");
	MSG(0, "    -r read only (to release unused blocks) for compressed "
			"files\n");
	MSG(0, "    ------------------------------------------------------\n");
	MSG(0, "  -d debug level [default:0]\n");
	MSG(0, "  -M maximum # of expandable sectors [default:0]\n");
	MSG(0, "  -V print the version number and exit\n");
	MSG(0, "  -X xattr_table_file to add custom xattrs\n");
	exit(1);
}
...
void f2fs_parse_options(int argc, char *argv[])
{
	...
	if (!strcmp("fsck.f2fs", prog)) {
		const char *option_string = ":aC:c:m:Md:fg:lL:O:p:q:StyV";
		int opt = 0, val;
		char *token;
		...
		while ((option = getopt_long(argc, argv, option_string,
			...
			case 'l':
				...
				break;
			case 'L':
				if (strlen(optarg) > MAX_VOLUME_NAME) {
...
#ifdef WITH_RESIZE
		const char *option_string = "d:fsF:t:iV";
		...
		const char *option_string =
			"cL:a:i:x:m:rn:C:d:f:p:s:St:T:X:VPM:";
		...
		while ((option = getopt(argc, argv, option_string)) != EOF) {
			...
			case 'n':
				c.compress.required = true;
				c.compress.filter_ops = &pc_filter;
				c.compress.filter_ops->add(optarg);
				break;
			...
			case 'T':
				...
				break;
			case 'X':
				c.xattr_table = xattr_table_open(optarg);
				if (!c.xattr_table) {
					MSG(0, "\tError: failed to load xattr_table_file: "
							"%s\n", optarg);
					exit(1);
				}
				break;
			case 'M':
				if (strncmp(optarg, "0x", 2))
					val = sscanf(optarg, "%"PRIu64"",
							&c.target_sectors);
				else
					val = sscanf(optarg, "%"PRIx64"",
							&c.target_sectors);
				ASSERT(val >= 0);
				break;
			case 'V':
		...
		if (err == NOERROR && c.compress.enabled) {
			...
			if (c.compress.filter == COMPR_FILTER_UNASSIGNED)
				c.compress.filter = COMPR_FILTER_ALLOW;
			else if (c.compress.filter_ops == &pc_filter) {
				MSG(0, "\tError: could not mix option -i and -x"
					" with -n\n");
				error_out(prog);
			}
...
{
	struct dump_option *opt = (struct dump_option *)c._private;
	struct f2fs_checkpoint *ckpt = F2FS_CKPT(sbi);
	u32 flag = le32_to_cpu(ckpt->ckpt_flags);
	...
	if (ret != 0) {
		if (ret == 1) {
			...
			stlog("Info: %s %u files, %u/%u nodes, %llu/%llu blocks, %u SPO cnt\n",
					c.devices[0].path, get_cp(valid_inode_count),
					get_cp(valid_node_count), sbi->total_node_count,
					get_cp(valid_block_count), get_cp(user_block_count),
					le32_to_cpu(fsck->extra_blk.spo_counter));
			c.elapsed_time = (get_boottime_ns() - start) / 1000000;
			fsck_set_extra_fs_info(sbi);
			fsck_update_extra_blk(F2FS_FSCK(sbi));
			stlog("Done: %llu msecs\n", c.elapsed_time);
			ret = 0;
		} else if (ret == -EAGAIN && retry_cnt) {
			retry_cnt = 0;
			f2fs_do_umount(sbi);
			c.fix_on = 1;
			MSG(0, "Error: Run fsck again w/ fix_on!\n");
...
retry:
	...
	xattr_table_close(c.xattr_table);

	c.elapsed_time = (get_boottime_ns() - start) / 1000000;
	c.exit_code = ret;
	if (c.func == FSCK)
		fsck_update_extra_blk(&gfsck);
