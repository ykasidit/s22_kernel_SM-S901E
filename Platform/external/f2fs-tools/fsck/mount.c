...
void update_free_segments(struct f2fs_sb_info *sbi)
{
	...
	MSG(1, "\r [ %c ] Free segments: 0x%x", progress[i % 5], get_free_segments(sbi));
	fflush(stdout);
	i++;
}
			...
			struct f2fs_node *node, int name)
{
	...
	if (name && en[0]) {
		...
		if (S_ISREG(le16_to_cpu(inode->i_mode)))
			type = "regular file";
		else if (S_ISDIR(le16_to_cpu(inode->i_mode)))
			type = "directory";
		else if (S_ISCHR(le16_to_cpu(inode->i_mode)))
			type = "character device";
		else if (S_ISBLK(le16_to_cpu(inode->i_mode)))
			type = "block device";
		else if (S_ISFIFO(le16_to_cpu(inode->i_mode)))
			type = "FIFO (named pipe)";
		else if (S_ISLNK(le16_to_cpu(inode->i_mode)))
			type = "symbolic link";
		else if (S_ISSOCK(le16_to_cpu(inode->i_mode)))
			type = "socket";
		else
			type = "unknown";
		MSG(0, " - File type         : %s\n", type);
...
printout:
	...
	DISP_u32(sb, crc);
	DISP_u32(sb, qf_ino[0]);
	DISP_u32(sb, qf_ino[1]);
	DISP_u32(sb, qf_ino[2]);
...
static struct cpflag cpflag_list[] = {
	{ CP_QUOTA_NEED_FSCK_FLAG, "quota_need_fsck" },
	{ CP_LARGE_NAT_BITMAP_FLAG, "large_nat_bitmap" },
	{ CP_NOCRC_RECOVERY_FLAG, "allow_nocrc" },
	{ CP_TRIMMED_FLAG, "trimmed" },
	{ CP_NAT_BITS_FLAG, "nat_bits" },
	{ CP_CRC_RECOVERY_FLAG, "crc" },
	{ CP_FASTBOOT_FLAG, "fastboot" },
	{ CP_FSCK_FLAG, "fsck" },
	{ CP_ERROR_FLAG, "error" },
	{ CP_COMPACT_SUM_FLAG, "compacted_summary" },
	{ CP_ORPHAN_PRESENT_FLAG, "orphan_inodes" },
	{ CP_DISABLED_FLAG, "disabled" },
	{ CP_RESIZEFS_FLAG, "resizefs" },
	{ CP_UMOUNT_FLAG, "unmount" },
	{ 0, 0 },
};
...
void print_cp_state(u32 flag)
{
	...
	sprintf(msg, "Info: checkpoint state = %x :", flag);
	for (f = cpflag_list; f->string; f++) {
		if (f->mask & flag)
			sprintf(msg + strlen(msg), " %s", f->string);
	}
	if (!(flag & CP_UMOUNT_FLAG))
		sprintf(msg + strlen(msg), " sudden-power-off");
 ...
 */
static int f2fs_should_proceed(struct f2fs_sb_info *sbi, u32 flag)
{
	struct f2fs_super_block *sb = F2FS_RAW_SUPER(sbi);
...
int sanity_check_ckpt(struct f2fs_sb_info *sbi)
{
	...
	if (!user_block_count || user_block_count >=
		...
		ASSERT_MSG("\tWrong user_block_count(%llu)\n", user_block_count);

		if (!f2fs_should_proceed(sbi, flag))
			return 1;
		if (!c.fix_on)
			return 1;

		if (flag & (CP_FSCK_FLAG | CP_RESIZEFS_FLAG)) {
			u64 valid_user_block_cnt;
			...
			MSG(0, "Info: Fix wrong user_block_count in CP: (%llu) -> (%llu)\n",
						...
						int want_type, bool new_sec)
{
	...
	if ((get_free_segments(sbi) <= SM_I(sbi)->reserved_segments + 1) &&
			!(get_sb(feature) & cpu_to_le32(F2FS_FEATURE_RO)))
		not_enough = 1;
...
bypass_ssa:
	...
	se = get_seg_entry(sbi, curseg->segno);
	if (c.zoned_model == F2FS_ZONED_HM || se->valid_blocks == curseg->next_blkoff)
...
static int tune_sb_features(struct f2fs_sb_info *sbi)
{
	...
	/* Check default mount opts */
	if (sb->feature & cpu_to_le32(F2FS_FEATURE_COMPRESSION))
		def_mnt_opts = "flush_group=5666,inlinecrypt,compress_log_size=5";
	else
		def_mnt_opts = "flush_group=5666,inlinecrypt";
	if (strncmp((char *)(sb->mount_opts), def_mnt_opts, strlen(def_mnt_opts))) {
		memset((char *)(sb->mount_opts), 0, sizeof(sb->mount_opts));
		strncpy((char *)(sb->mount_opts), def_mnt_opts, strlen(def_mnt_opts));
		MSG(0, "Info: mount_opts is set as %s\n", def_mnt_opts);
		sb_changed = 1;
	}
	if (strlen(c.vol_label)) {
		char buffer[MAX_VOLUME_NAME];

		utf16_to_utf8(buffer, sb->volume_name, MAX_VOLUME_NAME, MAX_VOLUME_NAME);

		if (strncmp(buffer, c.vol_label, strlen(c.vol_label))) {
			memset((char *)(sb->volume_name), 0, sizeof(sb->volume_name));
...
static void restore_backup_meta(struct f2fs_super_block *sb)
{
	...
	u32 target_segs = get_sb(segment_count_backupmeta);
...
static int disconnect_fsync_dnode(struct f2fs_sb_info *sbi)
{
	struct curseg_info *curseg;
	struct f2fs_node *node_blk;
	block_t blkaddr;
	...
	snprintf((char *)&node_blk->footer, sizeof(node_blk->footer),
		"fsck:Disconn node chain");

	err = dev_write_block(node_blk, blkaddr);
...
int f2fs_do_mount(struct f2fs_sb_info *sbi)
{
	...
	if (ret == -ENOMEM) {
		...
		ERR_MSG("record_fsync_data failed, disconnect fsync dnode\n");
		if (disconnect_fsync_dnode(sbi)) {
			ERR_MSG("disconnect fsync dnode failed\n");
			return -1;
		}
		return -EAGAIN;
	}
	...
	if (c.func == FSCK) {
		int unsigned spo_cnt;
		fsck_read_extra_flag_blk(sbi);
		spo_cnt = le32_to_cpu(fsck->extra_blk.spo_counter);
		if (!is_set_ckpt_flags(cp, CP_UMOUNT_FLAG)) {
			spo_cnt += 1;
			fsck->extra_blk.spo_counter = cpu_to_le32(spo_cnt);
		}
		stlog("Info: SPO %d/%d\n", spo_cnt, FSCK_SPO_CNT_THR);
	}

	if (!f2fs_should_proceed(sbi, get_cp(ckpt_flags)))
		return 1;
