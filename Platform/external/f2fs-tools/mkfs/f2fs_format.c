...
static int f2fs_prepare_super_block(void)
{
	...
	u_int32_t target_start_zone_offset;
	...
	if (c.max_nat)
		total_valid_blks_available  = min(c.max_nat, total_valid_blks_available);
...
static int f2fs_write_check_point_pack(void)
{
	...
	/* fix initial checkpoint version for debugging */
	cp->checkpoint_ver = cpu_to_le64(0x4321);
...
static int f2fs_write_super_block(void)
{
	...
	if (f2fs_fsync_device() < 0) {
		MSG(0, "\tError: Failed to fsync after write super block!!!\n");
		free(zero_buff);
		return -1;
	}
