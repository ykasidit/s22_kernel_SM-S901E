...
// < APE : start
// Import and enable both 'tc filter kind: bpf' and 'tc qdisc kind: clsact'
extern struct filter_util bpf_filter_util;
extern struct qdisc_util clsact_qdisc_util;
// > APE : end
...
// < APE : start
	else if (!strcmp(str, "clsact"))
		return &clsact_qdisc_util;
// > APE : end
